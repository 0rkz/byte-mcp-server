...
const PAYLOAD_ARCHIVE_BASE =
  process.env.PAYLOAD_ARCHIVE_BASE || "https://api.payperbyte.io/payload";
...
async function searchFactPublishers(opts: {
  ...
  if (!res.ok) {
    throw new Error(
      `Indexer publishers API ${res.status} at ${CONFIG.indexerUrl}. ` +
        `Ensure the indexer is reachable.`
    );
  }
  const data = (await res.json()) as PublisherSearchResult[];
 ...
 */
async function getRequestEndpoint(publisher: string): Promise<string | null> {
  ...
  try {
    ...
    const metadataRaw = (data as { metadata?: string }).metadata;
    if (!metadataRaw) return null;
    let meta: { request_endpoint?: string };
    try {
  ...
  while (Date.now() < deadline) {
    ...
    if (head > cursor) {
      const logs = await client.getLogs({
        ...
        args: { publisher: opts.publisher as `0x${string}` },
        fromBlock: cursor,
        toBlock: head,
      });
...
export async function queryFact(
  params: QueryFactParams
): Promise<FactResult | FactError> {
  ...
  if (publishers.length === 0) {
    return {
      error:
        "No fact-oracle publishers registered. Register one via byte-fact-oracle-feed and stake 100 PPB on DataRegistry.",
    };
  }
  ...
  for (const pub of publishers) {
    ...
    return {
      ...
      citations: Array.isArray(payload.citations)
        ? (payload.citations as FactResult["citations"])
        : [],
  ...
  return {
    error:
      "All fact-oracle publishers failed to deliver a matching response within the timeout. " +
