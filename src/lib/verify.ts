...
export function computePayloadHash(
  ...
  if (received instanceof Uint8Array) return keccak256(received).toLowerCase() as Hex;
  if (typeof received === "string") {
    return (received.startsWith("0x") && received.length % 2 === 0
      ? keccak256(received as Hex)
      : keccak256(toBytes(received))
    ).toLowerCase() as Hex;
  }
  // object → caller wants structured hashing
  return mode === "canonical"
    ? (keccak256(canonicalBytes(received)).toLowerCase() as Hex)
    : (keccak256(toBytes(JSON.stringify(received))).toLowerCase() as Hex);
}

function normalizeHash(h: string): Hex {
...
const BROADCAST_EVENT = parseAbiItem(
...
export async function recoverAttestationSigner(att: {
  ...
  deadline: bigint;
  signature: Hex;
}): Promise<{ signer: Address; match: boolean }> {
...
export async function verifyPayload(opts: {
  ...
  try {
    ...
    return {
      verified: false,
      recomputedHash: "0x" as Hex,
      onChainHash: "0x" as Hex,
      hashMatch: false,
      source: opts.txHash ? "txHash" : "expectedHash",
      reason:
        "could not hash the received payload (unserializable input, e.g. a BigInt) — fail-closed; pass JSON-serializable bytes/string",
    };
  }

  if (opts.txHash) {
    const receipt = await publicClient.getTransactionReceipt({
      hash: opts.txHash as Hex,
    });
    const log = receipt.logs.find(
      (l) =>
        l.address.toLowerCase() === ADDRESSES.DataStream.toLowerCase() &&
        l.topics[0] === keccak256(toBytes(
          "BroadcastStreamed(address,uint256,bytes32,uint256,uint256,uint256,uint256,bytes)",
        )),
    );
    if (!log) {
      return {
        ...
        reason:
          "no BroadcastStreamed event found in that tx on the r2 DataStream — pass expectedHash instead, or use the broadcast settlement tx",
      };
    }
    ...
    let signerMatch: boolean | undefined;
